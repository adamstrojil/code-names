...
const get25Words = (wordslist: Array<string>): Array<string> => {
  ...
  const result = shuffledWordsList.filter((_word, index) => index < 25).map(word => word.toUpperCase())
  return result
}
...
const assignRoles = (words:Array<string>):Array<WordCard> => {
  return words.map((word,index) => {
    ...
    return { 
      word,
      roleRevealed: false,
      role: index <= 8 ? 'red' : index > 8 && index <= 16 ? 'blue' : index > 16 && index <24 ? 'neutral' : 'black'
    }
  })
}
...
function App() {
  ...
  const wordsForRound:Array<WordCard> = assignRoles(get25Words(wordsList)).sort(() => Math.random() - 0.5)

  const toggleMode = () => gameMode === "single" ? setGameMode("mirrored") : setGameMode("single")
